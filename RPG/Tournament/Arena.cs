namespace CourseApp
{
    ...
    public class Arena
    {
        ...
        public void Tournament(int tournamentParticipants)
        {
            ...
            Logger.LoggerOutput("Бойцы предстоящего турнира!");
            foreach (Player item in warriors)
            {
                Logger.LoggerOutput($"Боец {item.Name}, {item.ClassPlayer}");
            }

            Logger.LoggerOutput("Наш турнир начинается!");
            ...
            Logger.LoggerOutput($"И так, Победитель турнира - {winners[0].ClassPlayer} {winners[0].Name}!");
        }
    }
}
