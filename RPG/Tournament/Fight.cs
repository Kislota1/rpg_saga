        ...
        public Tuple<Player, Player> Zaruba(Player warrior, Player warriorRival)
        {
            while ((warrior.Health > 0) && (warriorRival.Health > 0))
            {
                ...
                else if (warrior.Health > 0)
                {
                    ...
                    Logger.LoggerOutput($"{warriorRival.Name} теряет {warrior.DamageInfo} здоровья от удара {warrior.Name}!");
                }
                ...
                else if (warriorRival.Health > 0)
                {
                    ...
                    Logger.LoggerOutput($"{warrior.Name} теряет {warriorRival.DamageInfo} здоровья от удара {warriorRival.Name}!");
                }
            }
